...
<body>
	...
	<%
		...
		try {
			...
			String sql1 = "SELECT count(*) as ponum from purchaseorder where plant_id =" + id + "and '" + sdate
					+ "' <= enddate and startdate <= '" + edate + "'";
...
<script>
	...
	var tr = "";
	if (result == 0) {
		tr = "<h3>Dear user, plant is available in selected time range</h3>"
	} else {
		tr = "<h3>Dear user, plant is reserved in selected time range</h3>"
	}
	$('#lblmsg').append(tr);
</script>
